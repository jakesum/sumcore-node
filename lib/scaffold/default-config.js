...
/**
 * Will return the path and default sumcore-node configuration. It will search for the
 * configuration file in the "~/.litecoin" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.litecoin');
  var defaultConfigFile = path.resolve(defaultPath, './sumcore-node.json');
