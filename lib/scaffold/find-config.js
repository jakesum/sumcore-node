...
/**
 * Will return the path and sumcore-node configuration
...
function findConfig(cwd) {
  ...
  while (!fs.existsSync(path.resolve(directory, 'sumcore-node.json'))) {
  ...
  return {
    path: directory,
    config: require(path.resolve(directory, 'sumcore-node.json'))
  };
}
