...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'sumcore-lib': '^' + bitcore.version,
    'sumcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/sumcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the sumcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the sumcore-node directory and configuration
